      ...
      <aside className={`w-64 bg-white text-slate-700 flex flex-col h-screen fixed top-0 left-0 border-r border-slate-200/80 shadow-2xs z-40 font-sans transition-transform duration-300 ${
        ...
        <div className="h-20 flex items-center px-6 border-b border-slate-100 gap-3">
          ...
          <div>
            ...
            <span className="text-[10px] font-bold text-emerald-700 leading-tight block mt-1 tracking-wider uppercase">
              DEPENDENCY PORTAL
            </span>
          </div>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto py-5 px-3 space-y-1">
          ...
          {dependencyNavItems.map((item) => {
            const Icon = item.icon;
            const isActive = (item.href === '/dependency') 
              ? pathname === item.href 
              : pathname === item.href || (item.href === '/dependency/tasks' && pathname.startsWith('/dependency/') && pathname !== '/dependency/history');
        ...
        <div className="p-4 border-t border-slate-100 bg-white">
          ...
          >
            <div className="flex items-center gap-3 overflow-hidden">
              <div suppressHydrationWarning className="w-9 h-9 rounded-full bg-[#1e293b] text-white font-bold text-xs flex items-center justify-center shrink-0">
            ...
            <Settings className="w-4 h-4 text-slate-400 shrink-0 hover:text-slate-600" />
          </div>
        </div>
