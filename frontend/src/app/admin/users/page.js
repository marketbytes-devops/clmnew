...
export default function UsersList() {
  ...
  const filteredUsers = users.filter(u => {
    ...
    const roleName = (typeof u.role === 'object' && u.role !== null ? u.role.name : (typeof u.role === 'string' ? u.role : '')).toLowerCase();
    return name.includes(term) || email.includes(term) || roleName.includes(term);
  });
    ...
    <div className="flex flex-col gap-6 relative max-w-7xl mx-auto font-sans">
      ...
      <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-xs flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        <button 
          ...
          className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-gray-900 border border-gray-200 px-3 py-2 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer"
        >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-200 text-sm">
              {loading ? (
                <tr>
                  <td colSpan={5} className="p-8 text-center text-gray-400">Loading users from database...</td>
                </tr>
                    ...
                    <tr key={user.id || user.email} className="hover:bg-gray-50 transition-colors">
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          {displayAvatar ? (
                            <img src={displayAvatar} alt="User avatar" className="w-9 h-9 rounded-full object-cover border border-slate-200" />
                          ) : (
                      ...
                      <td className="p-4">
                        <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold ${
                          isActive ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-gray-100 text-gray-600'
                        }`}>
